...
def main():
    ...
    parser.add_argument(
        '-post_iter', help='Number of iterations', type=int, default=100000)
    parser.add_argument(
    ...
    parser.add_argument(
        '-post_lr', help='Initial learning rate', type=float, default=0.1)
    ...
    # model_wips = getattr(models, "WIPS")(opt.__dict__)

    # filtered_parameters = []
    # for name, param in model_wips.named_parameters():
    #     if param.requires_grad:
    #         filtered_parameters.append(param)
    # # print(filtered_parameters)
    # params_num = sum([np.prod(p.size()) for p in filter(
    #     lambda p: p.requires_grad, model_wips.parameters())])

    # model_wips = model_wips.cuda(opt.cuda)

    model_mdl_wips = getattr(models, "MDL_WIPS")(opt.__dict__)

    filtered_parameters = []
    for name, param in model_mdl_wips.named_parameters():
        if param.requires_grad:
            filtered_parameters.append(param)
    # print(filtered_parameters)
    params_num = sum([np.prod(p.size()) for p in filter(
        lambda p: p.requires_grad, model_mdl_wips.parameters())])

    model_mdl_wips = model_mdl_wips.cuda(opt.cuda)
    ...
    # train the model
    ret_dict = train.trainer_for_ablation(
        model_mdl_wips,
        dataset,
        lossfn,
        optimizer_adam,
    ...
    model_mdl_wips = ret_dict['best_rocauc_model_']
    ...
    # パラメータの置き換え
    opt.init_lr = opt.post_lr
    opt.iter = opt.post_iter
    ...
    # model_mdl_wips = getattr(models, "MDL-WIPS")(opt.__dict__)
